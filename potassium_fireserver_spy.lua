...
local guiParent = PL:WaitForChild("PlayerGui", 10) or PL:FindFirstChildOfClass("PlayerGui")
...
local function log(msg, color)
    ...
    for _, c in ipairs(scroll:GetChildren()) do
        if c:IsA("TextLabel") then cnt = cnt + 1 end
    end
...
local function ser(v)
    ...
    if t == "table" then
        local p = {}
        for k,val in pairs(v) do p[#p+1] = tostring(k).."="..ser(val) end
...
copyBtn.MouseButton1Click:Connect(function()
    ...
    task.delay(1.5, function()
        if copyBtn and copyBtn.Parent then copyBtn.Text = "Copy All" end
    end)
end)
...
task.spawn(function()
    ...
    -- outgoing: hookmetamethod (Synapse/Potassium native, avoids readonly issues)
            ...
            if active and (method == "FireServer" or method == "InvokeServer") then
                pcall(function()
                    ...
                    if (ok1 and isRE) or (ok2 and isRF) then
                        ...
                        local args = {...}
                        local p = {}
                        for i,v in ipairs(args) do p[i] = ser(v) end
                        local prefix = (ok2 and isRF) and "[RF->] " or "[->] "
                        log(prefix..path.." | "..table.concat(p,", "), Color3.fromRGB(255,220,80))
                    end
                end)
            end
            return oldNC(self, ...)
    ...
    log("hookmetamethod: "..(ncOk and "OK - outgoing captured" or "FAILED"))
    ...
    local function hookAll(root)
        pcall(function()
            for _,v in ipairs(root:GetDescendants()) do
                if v:IsA("RemoteEvent") then hookRE(v) end
            end
    ...
    local cnt = 0
    for _ in pairs(hooked) do cnt = cnt + 1 end
