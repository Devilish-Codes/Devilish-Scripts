-- Hierarchy Viewer | Delta Executor
...
-- Also print to executor console as fallback
print("[HierarchyViewer] " .. nodeCount .. " nodes found")

-- ── GUI ──────────────────────────────────────────────────────────────────────
local ok, err = pcall(function()

	local LINE_HEIGHT = 14
	local textHeight  = math.max(500, #lines * LINE_HEIGHT)
	...
	-- patch bottom of bar so it doesn't show rounded corners mid-window
	local patch = Instance.new("Frame")
	patch.Size             = UDim2.new(1, 0, 0, 6)
	patch.Position         = UDim2.new(0, 0, 1, -6)
	patch.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
	...
	textBox.Size                   = UDim2.new(1, -8, 0, textHeight)
...
if not ok then
	warn("[HierarchyViewer] GUI error: " .. tostring(err))
	warn("[HierarchyViewer] Copying hierarchy to clipboard instead")
	setclipboard(fullText)
end
